...
class _ProfilPageState extends State<ProfilPage> with TickerProviderStateMixin {
  ...
  bool musicEnabled = true; // ✅ Initialisé à true par défaut
  ...
  Future<void> _loadSettings() async {
    ...
    // ✅ CORRECTION : Synchroniser AVANT de mettre à jour l'état
    bool savedMusicEnabled = prefs.getBool('music') ?? true;

    setState(() {
      notificationsEnabled = prefs.getBool('notifications') ?? true;
      musicEnabled = savedMusicEnabled; // ✅ Utiliser la valeur sauvegardée
      currentPassword = prefs.getString('currentPassword') ?? '';
      totalScore = prefs.getInt('totalScore') ?? 245;
    });

    // ✅ CORRECTION : Synchroniser avec AudioService APRÈS la mise à jour de l'état
    await AudioService.setMusicEnabled(savedMusicEnabled);

    print("🎵 Profil - État musique chargé: $savedMusicEnabled");
  }
  ...
  Future<void> _saveSettings() async {
    ...
    // ✅ CORRECTION : Synchroniser avec AudioService
    await AudioService.setMusicEnabled(musicEnabled);

    print("🎵 Profil - État musique sauvegardé: $musicEnabled");
  }
