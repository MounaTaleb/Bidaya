...
class _CountingSticksGamePageState extends State<CountingSticksGamePage> {
  ...
  // Initialisation avec des valeurs par défaut pour éviter LateInitializationError
  int correctAnswer = 1;
  List<int> answerOptions = [1, 2, 3, 4, 5, 6];
  int? selectedAnswer;
  ...
  Future<void> _saveScores() async {
    if (_prefs != null) {
      ...
      await _prefs!
          .setInt('counting_sticks_correct_answers', correctAnswersCount);
      await _prefs!
          .setInt('counting_sticks_total_questions', totalQuestionsCount);
    }
  }
  ...
  Color _getAnswerButtonColor(int number) {
    ...
    if (number == correctAnswer) return Colors.green.shade300;
    if (number == selectedAnswer && !isCorrect) return Colors.red.shade300;
  ...
  Widget build(BuildContext context) {
    if (_prefs == null) {
      // Afficher un loader tant que les SharedPreferences ne sont pas chargées
                  ...
                  children: [
                    ...
                    Wrap(
                      ...
                      children: answerOptions
                          .map((number) => _buildAnswerBubble(number))
                          .toList(),
                    ),
                    const Spacer(),
